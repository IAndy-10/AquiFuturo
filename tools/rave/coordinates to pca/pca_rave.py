...
def features_to_latent_pca(model, z_scores):
    """ Reference: https://www.askpython.com/python/examples/principal-component-analysis"""

    """ Step 1: Center the data """
    model_meaned = model - np.mean(model, axis=0)

    """ Step 2: Calculate the covariance matrix """
    cov_mat = np.cov(model_meaned , rowvar = False)
    ...
    sorted_eigenvalue = eigen_values[sorted_index] # Fancy indexing
    ...
    model_reduced = np.dot(eigenvector_subset.transpose(), model_meaned.transpose()).transpose()

    """ Step 7: Map the node features to the PCA space """
    node_features = np.dot(z_scores, model_reduced.transpose())
...
def main():
    ...
    Z = features_to_latent_pca(traj, latent_dim, args.z_scale, mean, std)
