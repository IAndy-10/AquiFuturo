...
> **v1.1 change summary.** The audio layer is simplified from per-node 3D spatialised
> emitters to a **flat multi-track mix** (stereo/2D sources) whose low-pass filtering and
> stereo panning are driven by device pose. HRTF spatialisation (Steam Audio) is removed.
> One of the tracks is a RAVE-rendered stem; the rest are designed/recorded. No runtime
> inference. Affected sections: §1.2, §3, §4, §5.1–5.2, §6.5, §9, §10, §13, §17–§20, Appendix C.
...
**Framing note for the thesis:** the prototype does not claim to demonstrate real-time latent navigation. It demonstrates an embodied interface for a soundscape *whose primary voice was generated by graph-driven latent traversal*. Be precise about this distinction in the write-up — it is a defensible and honest scoping, but only if stated plainly.

### 1.3 Design intent

Contemplative, not game-like. The user is listening to a tree, not playing one. No score, no timers, no win state. The interaction vocabulary is: look, walk, tilt, touch.
...
Lock these versions on day one and record them in `README.md`. Version drift between the Unity MCP session and your local editor is a real failure mode.
...
┌─────────────── OFFLINE (desktop) ───────────────┐
│                                                  │
│  Photos ──► PostShot ──► point cloud ──► Blender │
│                                            │     │
│                                   ┌────────┴───┐ │
│                              tree mesh    root mesh
│                                   │            │ │
...
- Photograph the target tree: 150–300 images, orbit at 3 heights, overlap ≥ 60%, overcast light preferred, avoid wind.
- **Record with a tape measure:** trunk height to first branch, trunk diameter at breast height, canopy width. These are the scale references.
- Record GPS coordinates and a site photo for the thesis appendix (not used in-app).
- **While you are there, record audio** for the `soil` and `canopy` tracks — contact mic on the trunk, ambient stereo pair under the canopy. Same site, same tree. This costs you one extra hour on capture day and saves a separate trip.

### 6.2 PostShot `[ME]`

Produce a point cloud / splat, export point cloud as `.ply` to `assets_src/pointclouds/`.
